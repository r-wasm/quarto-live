...
```{webr}
filtered_cars <- mtcars |> dplyr::filter(hp > 123)
ojs_define(filtered_cars)
```
...
## Editable R code blocks

```{webr}
#| edit: true
edit_cars <- mtcars |> dplyr::filter(hp < 80)
head(edit_cars, n = 5)
hist(edit_cars$wt)
ojs_define(edit_cars)
```
...
Plot.plot({
  x: {label: "Economy (mpg)"},
  y: {label: "Power (hp)"},
  marks: [ Plot.dot(edit_cars, { x: "mpg", y: "hp"}) ]
})
```
